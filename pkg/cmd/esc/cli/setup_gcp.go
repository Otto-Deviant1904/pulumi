...
// gcpRoles maps the --policy choice to the IAM role granted to the service account. As with
// the other providers there is no default: editor is needed for Deployments and viewer for
// Insights.
var gcpRoles = map[string]string{
	"admin":    "roles/editor",
...
func createGCPEnvironment(
	...
	return applyProviderUpdate(ctx, setup.env, ref, "", path, node)
}
...
func newSetupGCPCmd(setup *setupCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "[EXPERIMENTAL] Set up GCP OIDC integration for Pulumi ESC\n" +
			...
			"\n" +
			"Examples:\n" +
			"  pulumi env setup gcp --policy admin\n" +
			"  pulumi env setup gcp --policy readonly --project-id my-project --yes\n",
		Args:         cobra.NoArgs,
		SilenceUsage: true,
	...
	cmd.Flags().StringVar(&policy, "policy", "",
		"the access level for the service account: admin (Editor, for Deployments) or readonly "+
			"(Viewer, for Insights); prompted for when omitted")
