...
// azureRoleIDs maps the --policy choice to the built-in role definition assigned per
// subscription. As with AWS there is no default: Contributor is needed for Deployments and
// Reader for Insights, so the choice is not a matter of degree.
var azureRoleIDs = map[string]string{
	// Contributor - https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/privileged#contributor
	"admin": "b24988ac-6180-42a0-ab88-20f7382dd24c",
	// Reader - https://learn.microsoft.com/azure/role-based-access-control/built-in-roles/general#reader
	"readonly": "acdd72a7-3385-48ef-bd42-f606fba81ae7",
}
...
// azureEnvTarget pairs a subscription with the environment configured for it.
type azureEnvTarget struct {
	subscription cloudsetup.CloudAccount
	ref          environmentRef
}

// createAzureEnvironments writes one `fn::open::azure-login` OIDC block per subscription, each
// pinning its own subscription, reusing the same helpers as `env provider azure-login oidc`.
//
...
func createAzureEnvironments(
	...
	for _, t := range targets {
		fmt.Fprintf(setup.esc().stdout, "\nConfiguring environment %s for subscription %s (tenant %s):\n",
			t.ref.String(), t.subscription.ID, tenantID)

		node := buildAzureLoginOIDCNode(clientID, tenantID, t.subscription.ID, nil)
		if err := ensureProviderEnv(ctx, setup.env, t.ref, true); err != nil {
			fmt.Fprintf(setup.esc().stderr, "  %v\n", err)
			failed++
			continue
		}
		if err := applyProviderUpdate(ctx, setup.env, t.ref, "", path, node); err != nil {
			fmt.Fprintf(setup.esc().stderr, "  %v\n", err)
			failed++
			continue
		}
...
func newSetupAzureCmd(setup *setupCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "[EXPERIMENTAL] Set up Azure OIDC integration for Pulumi ESC\n" +
			...
			"\n" +
			"Examples:\n" +
			"  pulumi env setup azure --policy admin\n" +
			"  pulumi env setup azure --policy readonly --subscription <sub-id> --yes\n",
		Args:         cobra.NoArgs,
		SilenceUsage: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// federated credentials we would then be unable to reference.
			if envRef != "" && len(selected) > 1 {
				return errors.New("--environment cannot be used when setting up more than one subscription")
			}
			...
			for i, sub := range selected {
				ref := setup.env.parseRef(envRef)
				if envRef == "" {
					ref = setup.env.parseRef(
						fmt.Sprintf("%s/%s/%s", org, projectName, sanitizeEnvName(sub.Name, sub.ID)))
				}
				targets[i] = azureEnvTarget{subscription: sub, ref: ref}
			...
			for _, t := range targets {
				fmt.Fprintf(esc.stdout, "  subscription %s (%s): assign %s, environment %s\n",
					t.subscription.Name, t.subscription.ID, policy, t.ref.String())
			}
			if !yes {
	...
	cmd.Flags().StringVar(&policy, "policy", "",
		"the access level for the role: admin (Contributor, for Deployments) or readonly "+
			"(Reader, for Insights); prompted for when omitted")
	...
	cmd.Flags().StringVar(&envRef, "environment", "",
		"[<org>/][<project>/]<environment-name> to write; only valid for a single subscription")
