...
func evaluateResourceFile(
	...
	merged, filename, err := parseFile(
		ctx, path, fileType, inputFormat, res.Token, loadConverter, loaderTarget, packageDescriptor, inputFlags)
	if err != nil {
		return nil, err
	}
	return evaluateResourcePCL(merged, filename, fileType, res, evalContext, bindOpts...)
}

func evaluateResourcePCL(
	pclBytes []byte,
	filename, fileType string,
	res *schema.Resource,
	evalContext functionEvalContext,
	...
	return evaluatePCL(bytes.NewReader(pclBytes), filename, fileType, bind, evalContext)
}
