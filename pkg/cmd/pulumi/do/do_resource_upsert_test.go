...
// TestDoCmdResourceUpsertWithoutFileStillValidatesRequiredInputs verifies that making
// --input-file optional does not skip schema validation for required inputs.
//
//nolint:paralleltest // installMockUpsertBackend calls t.Setenv.
func TestDoCmdResourceUpsertWithoutFileStillValidatesRequiredInputs(t *testing.T) {
	mws, mlm := installMockUpsertBackend(t, &deploy.Snapshot{})
	stub := func(_ context.Context, _ *pflag.FlagSet, _ StatefulUpdateRequest,
	) (*StatefulUpdateResult, error) {
		require.Fail(t, "runStatefulUpdate should not be called when required inputs are missing")
		return nil, nil
	}
	loader := func(_ context.Context, _ *plugin.Context, _, source string) (plugin.Provider, error) {
		return &testProvider{spec: doResourceSpec(false)}, nil
	...
	cmd.SetArgs([]string{"azure:index:myResource", "upsert", "myres", "--yes", "--input", "pcl"})
	err := cmd.Execute()
	require.ErrorContains(t, err, "parse input file")
