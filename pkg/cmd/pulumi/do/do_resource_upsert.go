...
func (pc *packageCommand) newResourceUpsertCommand(res *schema.Resource) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Merge --input-* flags into the file's PCL AST so the resulting bytes are what we
			// both type-check and persist as the snippet body. If no file was provided, the flags
			// become the snippet body by themselves.
			inputFlags := collectInputFlags(cmd, "input", res.InputProperties)
			code, filename, err := parseFile(
			...
			// Bind + type-check the merged PCL against the resource schema so we surface schema
			// errors here rather than deep inside the engine's snippet runner.
			if _, err := evaluateResourcePCL(code, filename, "input", res, pc.evalContext()); err != nil {
