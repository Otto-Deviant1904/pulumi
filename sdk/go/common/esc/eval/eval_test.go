...
func (secretWrapperProvider) Open(
	ctx context.Context,
	inputs map[string]esc.Value,
	context esc.EnvExecContext,
) (esc.Value, error) {
...
func (swapRotator) Open(
	ctx context.Context,
	inputs, state map[string]esc.Value,
	context esc.EnvExecContext,
) (esc.Value, error) {
	return esc.NewValue(state), nil
}

func (swapRotator) Rotate(
	ctx context.Context,
	inputs, state map[string]esc.Value,
	context esc.EnvExecContext,
) (esc.Value, error) {
	...
	for _, e := range entries {
		...
		t.Run(e.Name(), func(t *testing.T) {
			...
			if accept() {
				...
				if doRotate {
					rotated, rotationResult, rotateDiags = RotateEnvironment(
						t.Context(),
						environmentName,
						env,
						rot128{},
			...
			var rotated *esc.Environment
			if doRotate {
				rotated_, rotationResult, diags := RotateEnvironment(
					t.Context(),
					environmentName,
					env,
					rot128{},
