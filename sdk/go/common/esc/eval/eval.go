...
type evalContext struct {
	...
	// rotateDocPaths is the subset of document paths to invoke rotation for when
...
func (e *evalContext) evaluateImport(expr ast.Expr, name string) (*value, bool) {
	...
	if imported, ok := e.imports[name]; ok {
		if imported.evaluating {
			e.diags.Extend(
				syntax.Error(
					expr.Syntax().Syntax().Range(),
					fmt.Sprintf("cyclic import of %v", name),
					expr.Syntax().Syntax().Path(),
				),
			)
			return nil, false
		}
...
func (e *evalContext) evaluateSkippedExpr(x *expr, accept *schema.Schema) (*value, bool) {
	...
	//
	// however, we also need to make sure the user has permission to access rotateOnly environments when they are editing
	// an environment to
	// avoid privilege escalation from adding a reference to an environment that they don't have access to, but the
	// scheduled rotator does.
	// therefore we will still evaluate rotateOnly imports when validating the root environment.
	//
	// we only do this for the root environment, because only root environments are rotated, and it is permissible for a
	// user to import a
	...
	if skipEval := accept.IsRotateOnly() && (!e.rotating && !e.validating || !e.isRootEnv); skipEval {
		return &value{
			def:        newMissingExpr(x.path, x.base),
...
// The accessor is of the form ["environments", $project, $env, ...], which is transformed into an import name in the
// form "$project/$env"
func (e *evalContext) evaluateEnvironmentReferenceAccess(
	x *expr,
...
func (e *evalContext) evaluateUnknownAccess(
	syntax ast.Expr,
...
func (e *evalContext) evaluateBuiltinRotate(x *expr, repr *rotateExpr) *value {
	...
	if !inputsOK || inputs.containsObservableUnknowns(e.rotating) || !stateOK || state.containsUnknowns() ||
		e.validating ||
		err != nil {
